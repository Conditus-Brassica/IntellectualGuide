...
class CRUDAgent(PureCRUDAgent):
    ...
    async def get_categories_of_region(self, json_params: Dict):
        async def session(region_name: str):
            async with self._kb_driver.session(database=self._knowledgebase_name) as session:
                return await self._reader.read_categories_of_region(session, region_name)

        # TODO Json parser

        return await asyncio.shield(session(json_params["region_name"]))

    async def get_landmarks_in_map_sectors(self, json_params: Dict):
        async def session(sector_names: List[str]):
            async with self._kb_driver.session(database=self._knowledgebase_name) as session:
                return await self._reader.read_landmarks_in_map_sectors(session, sector_names)

        # TODO Json parser

        return await asyncio.shield(session(json_params["sector_names"]))

    async def get_landmarks_refers_to_categories(self, json_params: Dict):
        async def session(sector_names: List[str]):
            async with self._kb_driver.session(database=self._knowledgebase_name) as session:
                return await self._reader.read_landmarks_refers_to_categories(session, sector_names)

        # TODO Json parser

        return await asyncio.shield(session(json_params["categories_names"]))

    async def get_landmarks_by_coordinates(self, json_params: Dict):
        async def session(coordinates: List[Dict[str, float]]):
            async with self._kb_driver.session(database=self._knowledgebase_name) as session:
                return await self._reader.read_landmarks_by_coordinates(session, coordinates)

        # TODO Json parser

        return await asyncio.shield(session(json_params["coordinates"]))

    async def get_landmarks_by_names(self, json_params: Dict):
        async def session(landmark_names: List[str]):
            async with self._kb_driver.session(database=self._knowledgebase_name) as session:
                return await self._reader.read_landmarks_by_names(session, landmark_names)

        # TODO Json parser

        return await asyncio.shield(session(json_params["landmark_names"]))

    async def get_landmarks_of_categories_in_region(self, json_params: Dict):
        async def session(region_name: str, categories_names: List[str]):
            async with self._kb_driver.session(database=self._knowledgebase_name) as session:
                return await self._reader.read_landmarks_of_categories_in_region(session, region_name, categories_names)

        # TODO Json parser

        return await asyncio.shield(session(json_params["region_name"], json_params["categories_names"]))

    async def get_landmarks_by_region(self, json_params: Dict):
        async def session(region_name: str):
            async with self._kb_driver.session(database=self._knowledgebase_name) as session:
                return await self._reader.read_landmarks_by_region(session, region_name)
...
if __name__ == '__main__':
    ...
    async def test():
        ...
        async with AsyncGraphDatabase.driver('bolt://localhost:7687', auth=('neo4j', 'ostisGovno')) as driver:
            ...
            task2 = asyncio.create_task(crud.get_landmarks_in_map_sectors({"sector_names": ["a1", "a2"]}))
            task3 = asyncio.create_task(crud.get_landmarks_refers_to_categories({"categories_names": ["озёра мядельского района", "национальные парки белоруссии"]}))
            task4 = asyncio.create_task(crud.get_landmarks_by_coordinates({"coordinates": [{"longitude": 26.91887917, "latitude": 54.84001}, {"longitude": 26.8629, "latitude":54.955}, {"longitude":26.8684 , "latitude":54.9683}]}))
            task5 = asyncio.create_task(crud.get_landmarks_by_names({"landmark_names": ["свирь", "рудаково", "нарочь"]}))
            ...
            task7 = asyncio.create_task(crud.get_landmarks_by_region({"region_name": "Мядзел"}))
